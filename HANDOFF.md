...
**待部署堆积（等 CEO 明示「推」）**：master 领先 origin 7 个 commit——42626fe 星座跨页导航 / 0b2167b 🌍地球真实自转轴 / ac13125 首页天图全屏 / eb813c3 探测器轨迹数据 / 040afbd 深空信使页 / c640eda 双向链接+地球卡 / +本批 5 船。一句「推」即 build dist → rsync 进 gh-pages worktree（`rsync -a --delete --exclude='.nojekyll' --exclude='.git'`）→ commit+push gh-pages。
