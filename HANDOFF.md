...
- **批1b 页面 🔄（subagent 建中）**：`app/public/messengers.html`，three.js 俯视场景（sqrt 径向压缩）+ 双轨迹 + 里程碑 + 日球层顶环 + 时间轴动画。制作人（我）亲验 + live 浏览器 QA + CEO 看动画拍板。
- **批1c 首页注明 + 导航互链**：页面通过后加首页 demo 注明 + 各页 topnav 加「深空信使」链接。
