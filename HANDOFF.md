...
- [ ] **B-W 冬季星空**：6 锚点（冬季大三角/天狼星/马头星云/玫瑰星云/毕宿五与毕星团/M35）
      + 天区列表按「夏季银河/冬季星空/南天深空」分组 + 冬季大三角连线 overlay
      + anchors 加 `group`/`tour` 字段。坐标已用 CDS Sesame 解析（见 data 文件）。
      预设裁定：自动漫游维持原 10 站（`tour:true`）直到 B-T 上配置。
- [ ] **B-S 南天深空**：6 锚点（南十字与煤袋/半人马座ω/杜鹃座47/蜘蛛星云/南门二·比邻星/
      船帆座超新星遗迹）。纯数据+文案，UI 复用 B-W 分组。
