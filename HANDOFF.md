...
Updated: 2026-07-06（**新功能：深空信使 / Deep Space Messengers 页（进行中）** + 三项待部署堆积中）
...
- **批1c 双向链接（进行中）**：CEO 要本页每颗行星→跳到 `solar.html` 对应卡片（滚动+高亮），太阳系各页加「深空信使 →」一键回来。太阳系是**单页**（月球/火星各一块 + 行星画廊卡片），无一颗一页、无太阳/地球块——先接 6 行星(水/金/木/土/天/海)+火星共 7 个；太阳/地球待 CEO 定是否补介绍卡。用跳卡不用 popup（复用现成内容、真双向、不重复造）。
...
**待部署堆积（等 CEO 明示「推」）**：master 领先 origin 3 个 commit——42626fe 星座跨页导航 / 0b2167b 🌍地球真实自转轴 / ac13125 首页天图全屏。dist 已 build+rsync 进临时 worktree `scratchpad/ghp-deploy`（staged 未 commit），一句「推」即 commit+push gh-pages（记得 .nojekyll 已排除保住）。
