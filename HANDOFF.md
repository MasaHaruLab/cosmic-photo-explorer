...
- [ ] **B-V 深空信使（CEO 2026-07-02 追加：卫星+轨迹银丝带）**：旅行者 1/2、先驱者
      10/11、新视野号——只有深空探测器在天图上有"确定的一个点"；用 JPL Horizons API
      拉历年 RA/Dec 采样烘焙成静态 JSON，天图上画银白丝带轨迹 + 端点当前位置，点击
      真飞过去。诚实边界：绕地卫星（哈勃/悟空号）90 分钟扫全天钉不住 → 归 B-E 图文区；
      L2 的韦布/Gaia 可画年度绕黄道漂移带。图片搜寻派非 Anthropic 池（CEO 指示）。
- [ ] **B-E 观星台页「太空里的眼睛」**：新区块——哈勃/韦布/悟空号 DAMPE/Gaia 卫星本尊。
      事实先 WebSearch 验证（悟空号发射时间/机构/官网），配图走授权链（NASA 公版/ESA）。
- [ ] **B-P 太阳系（CEO 追加："不能忘了自己星系"+真实穿越感）**：月球/火星用 CDS 行星
      HiPS——Aladin 能真的拉近到表面（真·穿越）；其余行星 NASA 高清照 + 应用内灯箱。
      先 spike 验证行星 HiPS 在 Aladin Lite v3 嵌入模式可用，再定页面形态。
- [ ] **B-T 自动漫游可配置**：分组勾选 + 停留时长。背景乐裁定=不做（版权+自动播放策略坑）。
