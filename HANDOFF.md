...
## 闭环工具箱（常驻 · GOVERNANCE §12 四条腿 · 2026-07-10 补齐）

- **① 仪器**：`python3 scripts/check_data_invariants.py`（数据不变量：5 船齐、出发 r≈1 AU、里程碑下限、dist_au 与轨迹末端一致、last_contact 正确；改任何 data/ 或重抓星历后必跑）+ `cd app && npm run build`（构建门）。
- **② 冷验**：改动上线前按批派非作者冷验（惯例已在各批记录里，如批1a/批3 冷验通过条目）；视觉/动效类 CEO 前台实录（隐藏标签页 rAF 冻结验不了）。
- **③ 回滚**：每次部署在本文件记「回滚点 = 上一版 gh-pages `<hash>`」，`git push -f origin <hash>:gh-pages` 一条命令可退；src 靠 master 历史。**部署前先记回滚点，没记不推。**
- **④ 回写**：教训写进本文件对应批次条目（如录屏偏黑根因、GLOW 无回退必崩）；里程碑进 `项目日志.md`，当天进展进 `进展日志.md`。
...
CEO 拍板做一个独立新页「深空信使」——太空俯视视角，把探测器的**真实三维轨迹**从地球扇形画出来，标注真实掠行星里程碑 + 日球层顶边界，保留「卫星沿轨迹飞 + 时间轴」动画但科学准确。首页现有天图**不改**，只加一句注明「这是演示效果（视方向），真实轨道请点深空信使」。顺序：**先旅行者 → 提取 skill → 再做先驱者/新视野**。

**队列（按序）：**
- **批1a 数据地基 ✅（本 commit）**：`scripts/fetch_probe_vectors.py` 抓 NASA Horizons VECTORS（日心黄道 X/Y/Z，AU），出 `data/probe_vectors.json` + `app/public/` 副本。含旅行者 1&2 全程 + 真实里程碑（掠木/土/天/海、暗淡蓝点、冲出日球层）。**冷验通过**：两船均起于 r≈1.01 AU（地球轨道）、各掠行星半径与实际吻合、日球层顶穿越 V1=121.6/V2=119.0 AU（对上 2012/2018 实测）、V1 向黄道北 V2 向南。
- **批1b 页面 ✅（本 commit）**：`app/public/messengers.html`，three.js 场景（sqrt 径向压缩，BASE=6）+ 双轨迹 tube + 里程碑 + 日球层顶虚线环&网格球罩（CEO 特别喜欢，勿动）+ 星空背景。经多轮 live QA + CEO 反馈迭代：① **每艘探测器独立播放**（各自进度条/播放键/日期，弧长匀速，V1=40s/V2=46s 按弧长比例配速非对等）；② **行星按真实位置**（`data/planets.json`）+ **真实倾斜轨道**（真实 a/e/i/Ω/ω 画倾斜椭圆，非共面——CEO 要真实不要夸张，倾角小所以看着近乎扁盘=诚实现实）；③ **默认相机 ~40° 3/4 视角**（一眼看出行星盘面 + 探测器出黄道面：V1 北 V2 南）。
- **批1c 双向链接 ✅（本 commit）**：messengers 每颗行星名可点 → 跳 `solar.html#body-<key>`（滚动+高亮），solar 加「深空信使 →」导航链 + **右上角固定返回钮**（仅从 messengers 跳入时显形，滚到哪都在）。**新增地球卡**（Apollo 17「蓝色弹珠」，`planets/earth_bluemarble.jpg` + 3D 地球 `textures/2k_earth.jpg`），共 8 行星全接上。**深链滚动做稳**：原来在 DOMContentLoaded 就滚→被月球/火星查看器晚排版挤回顶部（=CEO 反馈「落在月球」）；改为等 `load` 后瞬时定位 + 350/900ms 两次纠偏 + 用户一滚即停。用跳卡不用 popup（复用现成内容、真双向、不重复造）。太阳/地球块：地球已补，太阳仍无块（CEO 只要地球）。
- **批1d 首页注明 ✅（本 commit）**：① 首页导航补「深空信使 →」徽章（原来缺，新页没接进首页入口）；② 探测器时间轴块加一句 `probe.realOrbitNote`：「地图上画的是从地球看过去的方向（演示效果）…真实三维轨道 → 深空信使」带 inline-link（点探测器才显）。天图本身不动。文案键进 `content.js`，样式 `.inline-link`/`.timeline-realorbit` 用探测器主题色 #9db8ff。
- **批3 其他探测器 ✅（本 commit）**：加**先驱者 10 号(-23)/11 号(-24)/新视野号(-98)**，共 5 船。`fetch_probe_vectors.py` 扩 3 条 PROBES（起始日 = 发射+1天，避开 SPK 覆盖起点几小时的整请求拒绝）；messengers.html 各补 `PROBE_COLORS`/`PROBE_GLOW`/`PROBE_SECONDS`（GLOW 无回退必须补否则崩），第780「现在」行去写死 v1/v2 改动态列全船。**冷验通过**：先驱10 首航木星5.05→末信号82.1、先驱11 首航土星9.38→末信号44.4、新视野 木星5.36→冥王星32.9→阿罗科斯43.3、各对上真实里程。文案从「两艘旅行者」外科扩成 5 船（旅行者专属事实明确标注）。渲染本就数据驱动，dock/chip/里程碑列表自动长出 5 行。**播放器日期改成活跃区间**：原来五船都显「现在位置」的 2026-07-06（轨迹外推到今，看着像今天集体撞墙），改成固定「发射年–末信号年」（先驱10=1972–2003/已失联、11=1973–1995/已失联、旅行者&新视野=…–至今/仍在运行）；数据加 `last_contact` 字段，拖动只动进度条不改日期。
- **批2 提取 skill ✅（先做批3 证明管线再固化）**：全局 skill `deep-space-probe-page`（`~/.claude/skills/`），两层——① 给本页加船的照单清单 ② 可迁移方法论（Horizons VECTORS 坐标系 / sqrt 径向压缩 / 弧长匀速配速 / 里程碑挂载 / 数据驱动 N 船）+ 真实踩坑清单；`references/known-probes.md` 存已验证的 Horizons ID+里程碑数据+候选船。已注册生效。

**✅ 已部署上线（2026-07-07）**：master `a4f09a3`（9 commit 全推 origin）+ gh-pages `7a54c1c`（deploy `9657555..7a54c1c`）。线上复验通过——`masaharulab.github.io/cosmic-photo-explorer/messengers.html` 正常 serving 五船新版。部署流程：build dist → `rsync -a --delete --exclude='.nojekyll' --exclude='.git' app/dist/ <ghp-worktree>/` → worktree `git add -A` + commit + push gh-pages + push master。worktree 在 `scratchpad/ghp-deploy`（.nojekyll/.git 已验证保住）。
