...
Updated: 2026-07-07（**深空信使页：批1a/1b/1c ✅（数据+页面+双向链接+地球卡）**，剩 1d 首页注明 / 批2 skill / 批3 其他探测器；+ 6 个 commit 待部署堆积）
...
- **批2 提取 skill**：把批1 沉淀成可复用 skill（数据管线 + 渲染 + 里程碑范式）。
- **批3 其他探测器**：用 skill 铺先驱者 10/11、新视野（数据脚本已含它们的 command，扩 milestones 即可）。

**待部署堆积（等 CEO 明示「推」）**：master 领先 origin 6 个 commit——42626fe 星座跨页导航 / 0b2167b 🌍地球真实自转轴 / ac13125 首页天图全屏 / eb813c3 探测器轨迹数据 / 040afbd 深空信使页 / +本批 双向链接+地球卡。一句「推」即 build dist → rsync 进 gh-pages worktree（`rsync -a --delete --exclude='.nojekyll' --exclude='.git'`）→ commit+push gh-pages。
