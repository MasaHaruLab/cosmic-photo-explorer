...
- **批3 其他探测器 ✅（本 commit）**：加**先驱者 10 号(-23)/11 号(-24)/新视野号(-98)**，共 5 船。`fetch_probe_vectors.py` 扩 3 条 PROBES（起始日 = 发射+1天，避开 SPK 覆盖起点几小时的整请求拒绝）；messengers.html 各补 `PROBE_COLORS`/`PROBE_GLOW`/`PROBE_SECONDS`（GLOW 无回退必须补否则崩），第780「现在」行去写死 v1/v2 改动态列全船。**冷验通过**：先驱10 首航木星5.05→末信号82.1、先驱11 首航土星9.38→末信号44.4、新视野 木星5.36→冥王星32.9→阿罗科斯43.3、各对上真实里程。文案从「两艘旅行者」外科扩成 5 船（旅行者专属事实明确标注）。渲染本就数据驱动，dock/chip/里程碑列表自动长出 5 行。
